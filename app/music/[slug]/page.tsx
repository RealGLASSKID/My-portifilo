...
const FALLBACK_TRACKS: Record<string, { title: string; meta: string; tag?: string }> = {
  "broken-dreams": { title: "AdoptedDreams", meta: "Single • 2024", tag: "NEW" },
  "lagos-nights": { title: "FreezingNights", meta: "Single • 2024" },
  "Survive-of-pain": { title: "Survive of Pain", meta: "Single • 2023" },
  "never-settle": { title: "Seperate Ways", meta: "Single • 2023" },
};
...
export default async function TrackPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const track = release
    ? { title: release.title, meta: `${release.type} • ${release.year}`, tag: release.tag }
    : FALLBACK_TRACKS[slug] ?? { title: slug, meta: "" };
  const hasLyrics = !!release?.lyrics && release.lyrics.length > 0;
  const audioUrl = release?.audioUrl;
        ...
        <div className="grid gap-8 md:grid-cols-[280px_1fr] md:items-center">
          <div className="relative aspect-square overflow-hidden rounded-2xl border border-white/5">
            {release?.coverUrl ? (
              ...
              <img src={release.coverUrl} alt={track.title} className="absolute inset-0 size-full object-cover" />
            ) : (
              <>
                <div
          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {audioUrl ? (
                ...
                <button
                  disabled
                  className="btn-glow inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold opacity-60"
                >
                  <Play className="size-4" /> Play Track
                </button>
              )}
